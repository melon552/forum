<template>
  <div class="tale-list-page">
    ...
    <!-- 帖子列表 -->
    <el-card class="tale-card">
      <el-skeleton v-if="loading" :rows="8" animated />
      <el-empty v-else-if="taleList.length === 0" description="暂无帖子数据" />
      ...
      <el-table
        ...
        class="table"
      >
        <el-table-column prop="taleId" label="帖子ID" width="80" />
...
import { getTaleList, updateTale, deleteTale } from '../../api';

// 搜索 & 分页
const searchKeyword = ref('');
const pagination = ref({ page: 1, limit: 10, total: 0 });
const originalTaleList = ref([]);
...
// 获取帖子列表（仅当前页）
const fetchTaleList = async () => {
  try {
    loading.value = true;
    const res = await getTaleList({
      page: pagination.value.page,
      limit: pagination.value.limit,
      keyword: '' // 不传 keyword，前端搜索
    });
    if (res.data?.code === 200) {
      taleList.value = res.data.data.list || [];
      originalTaleList.value = [...taleList.value];
...
const handleSearch = () => {
  ...
  taleList.value = k
    ? originalTaleList.value.filter(p =>
        p.title.toLowerCase().includes(k) ||
        p.author.toLowerCase().includes(k)
      )
    : [...originalTaleList.value];
};
...
const submitEditForm = async () => {
  try {
    ...
    const res = await updateTale(editForm.taleId, {
...
const handleDelete = async (row) => {
  try {
    ...
    const res = await deleteTale(row.taleId);
    if (res.data?.code === 200) {
