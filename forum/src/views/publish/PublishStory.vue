<template>
  <div class="publish-story-container">
    ...
    <el-form :model="form" :rules="rules" ref="formRef" label-width="80px" class="publish-form">
      ...
      <!-- 故事分类 -->
      <el-form-item label="故事分类" prop="type">
        <el-select 
          v-model="form.type" 
          placeholder="请选择故事分类" 
          class="input-item"
      ...
      <!-- 封面上传 -->
      <el-form-item label="故事封面" prop="cover">
        <el-upload
          class="upload-cover"
...
import { publishStory,saveDraft,draftDetail} from '../../api/index'; // 导入发布故事接口
...
//记录上次保存草稿的id
const LAST_DRAFT_KEY = 'last_story_draft_taleId'
// 表单数据
const form = reactive({
  title: '',
  type: '',
...
const handleSaveDraft = async () => {
  try {
    ...
    const res = await saveDraft(draftParams);
    console.log('保存草稿返回:', res);

    // 5. 处理返回结果（获取 taleId）
    if (res.data?.code === 200 && res.data.data?.taleId) {
      const taleId=res.data.data?.taleId
      localStorage.setItem(LAST_DRAFT_KEY,taleId)
      form.taleId = res.data.data.taleId; // 存储草稿ID（新建时赋值，编辑时更新）
      form.title = draftParams.title;
      form.type = draftParams.type;
      form.cover = draftParams.cover;
      ElMessage.success(res.data.msg || '草稿保存成功');
      // 跳转并携带 taleId，下次进入自动加载
      router.push({ 
        path: '/publishstory', 
        query: { taleId: form.taleId } 
      });
    } else {
...
const submitForm = async () => {
  try {
    ...
    const params = {
      taleId: form.taleId, // 草稿编辑后发布时携带，新建发布时为空
      title: form.title,
      type: form.type,
      cover: form.cover,
      content: htmlContent,
    ...
    // 5. 调用发布接口
    const res = await publishStory(params);
...
const fetchDraft=async ()=>{
  ...
  const taleId=route.query.taleId

  if(!taleId) return
  try {
    const res= await draftDetail(taleId)
    if(res.data?.code===200){
...
onMounted(async() => {
  ...
  const taleId = route.query.taleId;
  if (taleId) {
    form.taleId = taleId; // 赋值给 form，后续保存时会携带
